...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Cloudant (beta)](https://www.cdata.com/download/download.aspx?sku=EWZK-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Cloudant supported by the [CData JDBC Driver for Cloudant](https://www.cdata.com/drivers/cloudant/jdbc).
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/cloudant/download/jdbc](https://www.cdata.com/drivers/cloudant/download/jdbc)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Cloudant\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for Cloudant/`
    * Run the command `java -jar cdata.jdbc.cloudant.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.cloudant.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `cloudant.prp`) using the following properties and format:
    ...
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=cloudant
      ServerName=CDataCloudant
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.cloudant.jar
      DriverClass=cdata.jdbc.cloudant.CloudantDriver
      JdbcUrl=jdbc:cloudant:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
